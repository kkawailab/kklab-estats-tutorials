...
政府統計の総合窓口 **e-Stat** が提供する **API 機能（バージョン 3.0）** を使って、
日本の各種公的統計データを Python で取得・分析する方法を、初心者向けに解説したチュートリアルです。

- 全般的な使い方（API の仕組み・登録・リクエスト・レスポンス構造）をやさしく解説
- 各種データを入手する **30 個の実践例題**（人口・労働・物価・家計・GDP・産業・企業・教育など）
- 付録に **Flet（GUI）** と **FastAPI + HTMX（Web）** のサンプルアプリ
- Python の実行は **uv** ＋ **PEP 723**（インラインスクリプトメタデータ）形式
